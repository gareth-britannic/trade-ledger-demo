...
Want to try the browser application? Jump to [Run locally](#run-locally) to start the React frontend and its supporting API and local infrastructure.
...
### 2. Start the local stack, API, and React frontend

From the repository root, use two terminals. In Terminal 1, run the bootstrap script and wait for it to finish. Then load the generated Cognito configuration, supply the queue URL, and start the API:
...
In Terminal 2, after bootstrap has finished, install the locked frontend dependencies and start Vite:
...
`npm run dev` first creates the ignored `web/.env.local` from the public Cognito values generated by bootstrap, then starts Vite. On later runs, `npm ci` only needs to be repeated when the lockfile or installed dependencies change.

Open <http://127.0.0.1:5173/sign-in> and use these disposable local credentials:

| Field | Value |
| --- | --- |
| Email | `demo@trade-ledger.local` |
| Password | `TradeLedgerDemo123!`, unless overridden with `TRADE_LEDGER_LOCAL_USER_PASSWORD` before bootstrap |

After signing in, use **+ Add fill** to queue a trade, **View lots** to inspect FIFO lots, and **Explain** to ask a ledger question. There is intentionally no registration or password-reset flow. Authentication demonstrates access-token issuance and validation, but every authenticated identity sees the same shared demo ledger; the application is not multitenant.
...
With both terminals running, the local endpoints are:

- React sign-in: <http://127.0.0.1:5173/sign-in>
...
At the time of writing this passes 127 unit tests and 17 non-external API/integration tests. Four external integration tests are explicitly skipped during the ordinary run unless they are enabled.
