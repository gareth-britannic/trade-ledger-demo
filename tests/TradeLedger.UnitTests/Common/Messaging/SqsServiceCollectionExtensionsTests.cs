...
{
    [Fact]
    public void AddSqsClient_ValidLocalConfiguration_RegistersConfiguredSqsClient()
    {
        var services = new ServiceCollection();
        services.AddSqsClient(Configuration(ValidLocalConfiguration()), Environment(Environments.Development));
    ...
    [Fact]
    public void AddSqsClient_LocalConfigurationInProduction_ThrowsWhenClientIsResolved()
    {
        var services = new ServiceCollection();
        services.AddSqsClient(Configuration(ValidLocalConfiguration()), Environment(Environments.Production));
        using var provider = services.BuildServiceProvider();
    ...
    [Fact]
    public void AddSqsClient_IncompleteLocalConfiguration_ThrowsWhenClientIsResolved()
    {
        var configuration = ValidLocalConfiguration();
        configuration.Remove("AWS:SecretKey");
        var services = new ServiceCollection();
        services.AddSqsClient(Configuration(configuration), Environment(Environments.Development));
        using var provider = services.BuildServiceProvider();
    ...
    [Fact]
    public void AddSqsClient_InvalidQueueUrl_FailsOptionsValidation()
    {
        var configuration = ValidLocalConfiguration();
        ...
        Should.Throw<OptionsValidationException>(() =>
            provider.GetRequiredService<IOptions<SqsQueueOptions>>().Value);
    }

    [Fact]
    public void AddSqsClient_MissingLocalServiceUrl_AllowsDefaultAwsConfiguration()
    {
        var configuration = ValidLocalConfiguration();
        configuration.Remove("AWS:ServiceUrl");
        var services = new ServiceCollection();
        services.AddSqsClient(Configuration(configuration), Environment(Environments.Production));
        using var provider = services.BuildServiceProvider();
