...
{
    public static LedgerReplayResult Replay(IReadOnlyList<Fill> fills)
    {
        ArgumentNullException.ThrowIfNull(fills);

        IReadOnlyList<Lot> openLots = new List<Lot>();
        var realisedPnlEntries = new List<RealisedPnlEntry>();
        var realisedPnl = 0m;
        foreach (var fill in fills)
        {
            if (fill.Side == Side.Buy)
            {
                openLots = ApplyBuy(openLots, fill);
                continue;
            }

            var match = ApplySell(openLots, fill);
            openLots = match.RemainingLots;
            realisedPnl += match.RealisedPnl;
            realisedPnlEntries.Add(new RealisedPnlEntry(
                fill.Id,
                fill.Symbol,
                match.RealisedPnl,
                fill.ExecutedAt));
        ...
        return new LedgerReplayResult(openLots, realisedPnlEntries, realisedPnl);
    }
